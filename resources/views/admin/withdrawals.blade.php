    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Withdrawals</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Pending withdrawal requests</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">Pending Withdrawals</h3>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($pendingWithdrawals as $withdrawal)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900">AED {{ number_format($withdrawal->amount, 2) }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        <div class="text-xs text-gray-500">{{ $withdrawal->user->bank_name ?? '—' }}</div>
                                        <div class="text-sm font-semibold">{{ $withdrawal->user->bank_account_number ?? '—' }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $withdrawal->created_at->format('M d, Y H:i') }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <form method="POST" action="{{ route('admin.withdrawals.approve', $withdrawal) }}" class="inline-block">
                                                @csrf
                                            ...
                                            <form method="POST" action="{{ route('admin.withdrawals.reject', $withdrawal) }}" class="inline-block">
                                                @csrf
                            ...
                            @empty
                                <tr>
                                    <td class="px-6 py-6 text-center text-sm text-gray-500" colspan="5">No pending withdrawals.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
